...
def signup(request):
    if request.method == "POST":
        ...
        try:
            ...
            if user:
                return render(request, 'signup.html', {"form": SignUpForm(request), "message": "user already exist"})
            else:
                user = User.objects.create(username=username, password=make_password(password))
                login(request, user)
                return render(request, 'index.html')
        except User.DoesNotExist:
            return render(request, 'signup.html', {"form": SignUpForm(request), })
    return render(request, 'signup.html', {"form": SignUpForm(request), })
...
def concerts(request):
    ...
    try:
        is_authenticated = User.objects.get(username = username).is_authenticated
